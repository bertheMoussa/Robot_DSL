...
export class Robot implements Entities{
    ...
    speed:number //Millimeter per second
    ...
    turn(angle:number) : void {
        // Calculation the angle in radian
        this.rad = angle * Math.PI / 180;

        // Update of time elapsed
        const rotationTime=angle / this.speed * 1000;
        this.scene.time+=rotationTime;
        this.scene.timestamps.push(new Timestamp(this.scene.time,this));
    }

    move(dist:number) : void {
        // Calculation of robot coordinates
        const coord_x=Math.cos(this.rad)*dist;
        const coord_y=Math.sin(this.rad)*dist;
        ...
        // Update of time elapsed
        const moveTime=dist/this.speed*1000;
        this.scene.time+=moveTime;
        this.scene.timestamps.push(new Timestamp(this.scene.time,this));
    }

    side(dist:number) : void {
        // Set the Direction of the movement
        const rotationSideAngle = this.rad - Math.PI / 2;

        const coord_x = Math.cos(rotationSideAngle) * dist;
        const coord_y = Math.sin(rotationSideAngle) * dist;
